...
// import CartActions from "@/components/CartActions"; // We'll create this next
...
// // Main Server Component (no "use client"!)
// export default async function CartPage() {
//   // Read cart ID from cookies (server-side)

//   const cartId = await getCartIdServer();

//   // If no cart ID, show empty cart
//   if (!cartId) {
//     return (
...
//   // Fetch cart data from Shopify (server-side)
...
//             return (
//               <div key={line.id} className="flex gap-4 bg-white p-4 rounded-lg shadow">
//                 {/* Product Image */}
...
//                   <Link href={`/products/${product.handle}`} className="font-semibold text-black hover:text-blue-600">
//                     {product.title}
//                   </Link>
//                   <p className="text-sm text-black">{variant.title}</p>
...
//                 <CartActions lineId={line.id} initialQuantity={line.quantity} cartId={cartId} />
//               </div>
//             );
//           })}
//         </div>
...
//               {Number(cart.estimatedCost.totalAmount.amount).toLocaleString("en-US", {
//                 style: "currency",
...
//   const theme = (await searchParams)?.theme === "light" ? "light" : "dark";
