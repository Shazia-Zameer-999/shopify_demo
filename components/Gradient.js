...
/* NOTE:
   Removed `import { defineProperties } from "figma:react"` so this file is web-safe.
   If you need Figma metadata, either add it in a Figma-only file or keep a guarded call
   to `defineProperties` (shown at bottom) so the code won't crash in the browser/Next.
*/
...
export default function GradientShader({
  ...
  useEffect(() => {
    ...
    if (gl && programRef.current) {
      // If program is active, update uniforms right away
      if (uniforms.uSpeed) gl.uniform1f(uniforms.uSpeed, speed);
      if (uniforms.uLineCount) gl.uniform1f(uniforms.uLineCount, lineCount);
      if (uniforms.uYOffset) gl.uniform1f(uniforms.uYOffset, yOffset);
      if (uniforms.uAmplitude) gl.uniform1f(uniforms.uAmplitude, amplitude);
...
/* Guarded defineProperties call:
   If you're running inside Figma and `defineProperties` is injected, this will register controls.
   In the browser/Next it will be skipped, avoiding the runtime ReferenceError.
*/
